...
function tripTitle(input) {
  const raw = input.tripName ?? input.title;
  const text = String(raw ?? "").trim();
  return text || "Untitled trip";
}
...
function getYou() {
  return [...users.values()].find((user) => user.username === "you") || null;
}
...
function reset() {
  ...
  nextTripId = 1;
  seedDemoTravelers();
  createUser({ username: "you", displayName: "You" });
  recalculateRanks();
}
